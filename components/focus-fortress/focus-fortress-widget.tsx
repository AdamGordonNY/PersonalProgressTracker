...
interface FocusFortressWidgetProps {
  onClose?: () => void;
}

export function FocusFortressWidget({ onClose }: FocusFortressWidgetProps) {
  const { focusFortress } = useUIStore();
  ...
  if (!focusFortress.isMinimized && isActive) {
    // Full screen mode when active and not minimized
    return (
      <AnimatePresence>
        <motion.div
          initial={{ opacity: 0 }}
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        >
          {isActive ? "Stop Focus" : "Start Focus Session"}
        </Button>
      </div>
    </FloatingWidget>
  );
